...
QUAT_FACE_PANEL = [-0.7071, 0.0, 0.0, 0.7071]  # tool +Z along world +Y (adjust if approach looks wrong)
...
def main():
    ...
    node.get_logger().info("2/4 planning to hover pose (collision-aware)")
    moveit2.move_to_pose(position=HOVER, quat_xyzw=QUAT_FACE_PANEL)
    moveit2.wait_until_executed()

    node.get_logger().info("3/4 pressing (straight-line approach)")
    moveit2.move_to_pose(position=PRESS, quat_xyzw=QUAT_FACE_PANEL, cartesian=True)
    moveit2.wait_until_executed()

    node.get_logger().info("   holding 2 s on the button")
    import time; time.sleep(2.0)

    node.get_logger().info("4/4 retreating to hover")
    moveit2.move_to_pose(position=HOVER, quat_xyzw=QUAT_FACE_PANEL, cartesian=True)
    moveit2.wait_until_executed()
